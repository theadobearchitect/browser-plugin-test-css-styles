...
function CSSViewerMouseOver(e)
{
	...
	var hasAttr =  this.hasAttribute("data-assertionFailed") 
	if (!hasAttr) {
		return;
	}
...
function Custom_CSSViewerOutlineElements() {
	...
	for ( var ctr = 0; ctr < configJSON.assertions.length; ctr++) {
		var themeElements = document.querySelectorAll(configJSON.assertions[ctr].query); //document.getElementsByTagName("h1");
		if ( ! themeElements ) {
			return;
		}
		...
		for (var i=0;i<themeElements.length;i++) {
			...
			var element = document.defaultView.getComputedStyle(themeElement, null);
			//console.log(themeElement,element);
			if (themeElement.tagName != 'body') {
